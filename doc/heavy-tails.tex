...
github.com/carlislerainey/heavy-tails}
.}}\\\vspace{2mm}
...
%Mathematically, this suggests that applied researchers should not necessarily restrict themselves to unbiased estimators or minimum variance estimators under an assumed model. Instead, a more desirable criterion might be the mean squared error of the estimate (MSE) under a wide range of deviations from the assumed model. 
...
% We consider skewed error distributions as evidence that the basic structure of the model could be improved.
...
%This mean that there is approximately a one-half chance that $y |X$ falls above $\med(y | X)$ and a one-half changes that $y |X$ falls below.
...
Figure \ref{fig:cg-residuals-compare} compares the residuals from the least squares fit and the biweight fit after the Box-Cox transformation. Notice that the residuals tend to agree, with the exception of the 1980 election in Uganda, which is the largest residual in the biweight fit, but does not stand out among the least squares residuals.

\begin{figure}[h!]
\begin{center}
\includegraphics[scale = 0.8]{figs/cg-residuals-compare.pdf}
\caption{}\label{fig:cg-residuals-compare}
\end{center}
\end{figure}

Figure \ref{fig:cg-weights} presents the 35 smallest weights from the biweight fit. Recall that as cases become increasingly inconsistent with the majority of the data, the biweight estimator increasingly down weights the cases, potentially to zero. As we might expect, given the residuals in Figure \ref{fig:cg-residuals-compare}, only the 1980 election in Uganda receives zero weight. This election is highly consistent with the hypothesis because it features single member districts, an extremely large number of ethnic groups, and featured only two major political parties. Unfortunately for the hypothesis, this case is inconsistent the the majority of the data.

\begin{figure}[h!]
\begin{center}
\includegraphics[scale = 0.8]{figs/cg-weights.pdf}
\caption{}\label{fig:cg-weights}
\end{center}
\end{figure}
...
%\subsubsection*{Questions About Measuring ``Social Heterogeneity''}
