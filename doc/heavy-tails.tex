...
Dan Baissa\symbolfootnote[2]{Dan Baissa is a Ph.D. student in the Department of Government, Harvard University, 1737 Cambridge St., Cambridge , MA, 02138 (\href{mailto:dbaissa@g.harvard.edu}{dbaissa@g.harvard.edu}).}
...
However, by assuming A1, A2, A3, and A4, we obtain the minimum variance unbiased estimator (MVUE), which has the smallest possible variance among the class of unbiased estimators (e.g., \citealt{Wooldridge2013}, pp. 809-815).
...
Researchers have primarily justified least squares primary by the Gauss-Markov theorem because it seems to impart desirable properties without the overly restrictive assumption of normal errors.
...
The Gauss-Markov theorem has convinced researchers in political science that as long as A1, A2, and A3--Gauss-Markov assumptions--are met, the distribution of the errors is unimportant. 
...
\caption{This table shows the quantities of interest from least squares and biweight estmates, with and without the Box-Cox transformation of the outcome variable. 
...
In this paper, we adopted and defended a skeptical perspective toward least squares and explained the importance of carefully scrutinizing residuals.
We first noted that the restriction to linear estimators, as required by the Gauss-Markov theorem is unnecessary, artificial, and counterproductive. 
When the errors do not follow a normal distribution, restricting ourselves to linear estimators limits our ability to efficiently estimate the model coefficients. And as \cite{BerryFeldman1985} note, normal errors is usually difficult to defend using either theory or data. 
