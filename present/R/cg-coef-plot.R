...
pdf("present/figs/cg-coef-plot-1.pdf", height = 4, width = 5)
par(mfrow = c(1, 1), oma = c(3, 12, 2, 1), mar = c(1, 1, 1, 1))
...
### do the plotting
n.coef <- length(coef(ls.90s))
pdf("present/figs/cg-coef-plot-2.pdf", height = 4, width = 5)
par(mfrow = c(1, 1), oma = c(3, 12, 2, 1), mar = c(1, 1, 1, 1))
eplot(xlim = c(-5, 7), ylim = c(n.coef + 0.5, 0.5),
      yat = 1:n.coef,
      yticklab = c("Constant",
